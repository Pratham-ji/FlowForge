...
createWorkflowInstanceUC wRepo iRepo orgId uId role wfId instId = runExceptT $ do
  checkPerm role CreateInstance uId
  w <- ExceptT $ getWorkflow wRepo orgId wfId
  inst <- runDomain $ createWorkflowInstance w instId uId
  -- version 1
...
executeWorkflowTransitionUC
  ...
  -> WorkflowAction
  -> m (Either AppError ())
executeWorkflowTransitionUC txPort wRepo iRepo aRepo orgId uId role instId action = runExceptT $ do
  -- Authorize overall read access (Transition function also checks specific edge permissions)
  checkPerm role TransitionInstance uId
  ...
  ExceptT $ withTransaction txPort $ runExceptT $ do
    ...
    (newInst, audit) <- runDomain $ transition w inst uId role action
...
getWorkflowInstanceUC
  ...
  -> WorkflowInstanceId
  -> m (Either AppError WorkflowInstance)
getWorkflowInstanceUC iRepo orgId uId role instId = runExceptT $ do
  checkPerm role ReadInstance uId
...
getAuditHistoryUC aRepo orgId uId role instId = runExceptT $ do
  checkPerm role ReadAudit uId
  ExceptT $ getAuditHistory aRepo orgId instId
